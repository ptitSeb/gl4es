...
#ifdef TEXSTREAM
	    if (globals4es.texstream && (target==GL_TEXTURE_2D) && (width>=256 && height>=256) && 
...
void gl4es_glTexSubImage2D(GLenum target, GLint level, GLint xoffset, GLint yoffset,
    ...
    gltexture_t *bound = glstate->texture.bound[glstate->texture.active][itarget];
    if(!bound) {
        printf("LIBGL: No texture bound on unit %d for itarget=%d for TexSubImage\n", glstate->texture.active, itarget);
        return;
    }
    if (globals4es.automipmap) {
        if (level>0)
...
#endif
    {
        if (!pixel_convert(old, &pixels, width, height, format, type, bound->inter_format, bound->inter_type, 0, glstate->texture.pack_align)) {
