    ...
    */
    ...
    switch(pname) {
        case GL_TEXTURE_GEN_MODE: {
            ...
            switch (coord) {
                case GL_S: glstate->texgen[glstate->texture.active].S = param[0]; if(mode!=-1) { glstate->fpe_state->texgen_s_mode&=~(7L<<n); glstate->fpe_state->texgen_s_mode|=(((uint64_t)mode)<<n); } break;
                case GL_T: glstate->texgen[glstate->texture.active].T = param[0]; if(mode!=-1) { glstate->fpe_state->texgen_t_mode&=~(7L<<n); glstate->fpe_state->texgen_t_mode|=(((uint64_t)mode)<<n); } break;
                case GL_R: 
                    if(param[0]==GL_SPHERE_MAP) {
                        errorShim(GL_INVALID_ENUM);
                        return;
                    }
                    glstate->texgen[glstate->texture.active].R = param[0]; if(mode!=-1) { glstate->fpe_state->texgen_r_mode&=~(7L<<n); glstate->fpe_state->texgen_r_mode|=(((uint64_t)mode)<<n); } break;
                case GL_Q: 
                    if(param[0]==GL_REFLECTION_MAP || param[0]==GL_NORMAL_MAP || param[0]==GL_SPHERE_MAP) {
                        errorShim(GL_INVALID_ENUM);
                        return;
                    }
                    glstate->texgen[glstate->texture.active].Q = param[0]; if(mode!=-1) { glstate->fpe_state->texgen_q_mode&=~(7L<<n); glstate->fpe_state->texgen_q_mode|=(((uint64_t)mode)<<n); } break;
                default:
                    errorShim(GL_INVALID_ENUM);
            }
            return;
